    ...
    <div>
      ...
      <div className="flex w-full justify-between border border-gray-300 border-solid border-1px py-4 px-[6.8%]">
        <div className="flex items-center text-[#1C1C1C]">
          <Image src="/menu.svg" width="18" height="12" alt={""} className="mr-4"/>
          Tất cả
        </div>
        <div>
          <ul className="flex items-start">
            {navItems.map((item, index) => (
              <li className="px-4" key={index}>
                <a className="text-[#1C1C1C]" href={item.name}>{item.name}</a>
              </li>
            ))}
            <li className="flex px-4">
                <a className="text-[#1C1C1C] mr-2" href="/more">Thêm</a>
